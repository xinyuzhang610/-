<template>
  <div class="teacher-guidance">
    <header class="page-hero"><span>NEED DISCOVERY</span><h1>把教学难题，变成清晰的工具路径</h1><p>四步完成需求定位。推荐来自规则与真实工具库，不把简单匹配包装成 AI。</p></header>
    <div class="guidance-map">
      ...
      <GuidanceStep :step="2" title="选择具体学科" :status="stepStatus(2)"><div class="choice-grid goals"><button v-for="name in (subjectsByCategory[category] || [])" :key="name" :disabled="!category" :aria-pressed="subject===name" @click="chooseSubject(name)"><strong>{{ name }}</strong></button></div></GuidanceStep>
      <GuidanceStep :step="3" title="明确教学目标" :status="stepStatus(3)"><div class="choice-grid goals"><button v-for="item in goals" :key="item.value" :disabled="!subject" :aria-pressed="goal===item.value" @click="chooseGoal(item.value)"><strong>{{ item.label }}</strong><small>{{ item.desc }}</small></button></div></GuidanceStep>
      <GuidanceStep :step="4" title="获得工具推荐" :status="stepStatus(4)">
...
async function loadRecommendations(){if(!category.value||!goal.value)return;loading.value=true;errorMessage.value='';try{if(demoEnabled.value){tools.value=getDemoData('recommendations').tools;return}const {data}=await getRecommendation({step:4,category:category.value,subject:subject.value||null,need_type:goal.value});tools.value=data.tools||[]}catch(e){errorMessage.value=e.response?.data?.detail||'无法连接推荐服务，请确认后端已启动。'}finally{loading.value=false}}
</script>
<style scoped>
