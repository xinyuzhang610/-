...
async function loadRecommendations(){if(!category.value||!goal.value)return;loading.value=true;errorMessage.value='';try{if(demoEnabled.value){tools.value=getDemoData('recommendations').tools;return}const {data}=await getRecommendation({step:3,category:category.value,need_type:goal.value});tools.value=data.tools||[]}catch(e){errorMessage.value=e.response?.data?.detail||'无法连接推荐服务，请确认后端已启动。'}finally{loading.value=false}}
</script>
<style scoped>
