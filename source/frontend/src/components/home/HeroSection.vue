  ...
  <section class="hero" :class="{ 'hero--still': animationSkipped }" data-narrative-section data-testid="hero-section">
    ...
    <div class="hero__content page-container">
      ...
      <div class="hero__actions" aria-label="选择进入方式">
        <RouterLink class="entry entry--teacher" to="/login?role=teacher" data-testid="teacher-entry">
          教师入口 <span aria-hidden="true">→</span>
        </RouterLink>
        <RouterLink class="entry entry--student" to="/student/guidance" data-testid="student-entry">
          学生入口 <span aria-hidden="true">→</span>
        </RouterLink>
      </div>
...
.hero__art { position: absolute; inset: 0; min-height: 46rem; }
...
.hero__content { position: relative; z-index: 2; display: flex; min-height: max(46rem, 100svh); flex-direction: column; justify-content: center; align-items: flex-start; padding-block: 8rem 5rem; }
...
.entry { display: inline-flex; min-height: 3rem; align-items: center; gap: 1.5rem; padding: .75rem 1.4rem; border: 1px solid var(--gold-400); border-radius: 999px; color: var(--moon-50); font-weight: 650; text-decoration: none; transition: background var(--duration-fast), color var(--duration-fast), transform var(--duration-fast); }
.entry--teacher { background: var(--gold-600); }
.entry--student { border-color: var(--jade-200); background: rgb(23 107 90 / 38%); }
.entry:hover { color: var(--moon-50); transform: translateY(-2px); }
...
@media (prefers-reduced-motion: reduce) { .hero__core { display: none; } .entry:hover { transform: none; } }
</style>
