...
h2 { margin-top: .8rem; font-family: var(--font-title); font-size: clamp(2.35rem, 4vw, 4.35rem); font-weight: 500; line-height: 1.12; letter-spacing: -.035em; transition: transform .3s var(--ease-out); cursor: default; }
...
h3 { max-width: 13em; margin: 1rem 0 2rem; font-family: 'KaiTi', 'STKaiti', 'AR PL UKai CN', serif; font-size: clamp(1.5rem, 2.2vw, 2.4rem); font-weight: 400; line-height: 1.3; transition: transform .3s var(--ease-out); cursor: default; }
h3:hover { transform: scale(1.06); }
ol { display: flex; flex-wrap: wrap; gap: .5rem; list-style: none; counter-reset: path; }
li { counter-increment: path; }
li a { display: inline-flex; min-height: 2.75rem; align-items: center; padding: .45rem .8rem; border: 1px solid rgb(248 250 245 / 32%); border-radius: 999px; color: var(--moon-50); text-decoration: none; transition: transform .3s var(--ease-out), border-color .3s, background .3s; }
li a:hover { transform: scale(1.1); border-color: var(--gold-400); background: rgb(213 166 79 / 12%); }
