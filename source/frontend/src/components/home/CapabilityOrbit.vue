...
.orbit__item { position: absolute; z-index: 2; display: flex; flex-direction: column; align-items: center; gap: .5rem; width: 9rem; text-align: center; color: var(--color-ink); text-decoration: none; }
.orbit__item::before { content: ''; display: block; width: 2.2rem; height: 2.2rem; border-radius: 50%; background: radial-gradient(circle at 35% 35%, rgb(255 255 255 / 50%), var(--c) 60%, transparent 100%); box-shadow: 0 0 14px var(--c), 0 0 35px var(--c); }
...
@media (hover: hover) { .orbit__item { transition: filter .3s; } .orbit__item::before { transition: transform .4s var(--ease-out), box-shadow .4s; } .orbit__item:hover { filter: brightness(1.3); z-index: 5; } .orbit__item:hover::before { transform: scale(1.4); box-shadow: 0 0 30px var(--c), 0 0 65px var(--c), 0 0 100px var(--c); } }
