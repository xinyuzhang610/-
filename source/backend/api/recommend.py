...
from services.recommend_service import recommend_tools
...
def get_recommendation(request: RecommendRequest, db: Session = Depends(get_db)):
    ...
    return RecommendResponse(step=request.step, message="已匹配到可解释推荐规则" if rule else "未命中专属规则，已提供同分类热门预设工具", tools=tools)
